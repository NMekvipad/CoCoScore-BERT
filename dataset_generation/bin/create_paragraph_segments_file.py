...
def parse_parameters():
    ...
    Reads the given (gzipped, sentence-level) segments file and writes a paragraph-level segments file.
    
    The output file is written to stdout and comes in a tab-delimited format with columns
    PMID, paragraph number, segment start byte position, segment end byte position
    ''')
    ...
    with gzip.open(segments_file, 'rt', newline='', encoding='utf-8', errors='strict') as fin:
        ...
        for row in csv.reader(fin, delimiter='\t'):
            ...
            if previous_pmid is None:
                ...
                previous_end = end
            elif pmid != previous_pmid or paragraph != previous_paragraph:
                output_row = (previous_pmid, previous_paragraph, previous_start, previous_end)
