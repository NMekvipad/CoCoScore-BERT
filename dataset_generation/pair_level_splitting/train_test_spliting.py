...
def split_train_test(df, test_fraction, seed):
    ...
    all_pairs = df['entity_pair']
    all_pairs = sorted(set(all_pairs))
    rs.shuffle(all_pairs)

    test_pairs_end_index = int(test_fraction * len(all_pairs))
    test_pairs = set(all_pairs[:test_pairs_end_index])

    is_test_pair = [pair in test_pairs for pair in df['entity_pair']]
    is_train_pair = np.logical_not(is_test_pair)
